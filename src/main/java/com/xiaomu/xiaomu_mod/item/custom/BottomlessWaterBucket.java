...
public class BottomlessWaterBucket extends Item {
    public BottomlessWaterBucket(Settings settings) {super(settings);}
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (Screen.hasControlDown()) {
                ...
                for (int j = 0; j < 5; j++){
                    for (int k = 0; k < 5; k++){
                        ...
                        if (world.getBlockState(pos_shift_1).isOf(Blocks.AIR) | world.getBlockState(pos_shift_1).isOf(Blocks.VOID_AIR)) {
                            world.setBlockState(pos_shift_1, Blocks.WATER.getDefaultState());
                        }else if (world.getBlockState(pos_shift_1).isOf(Blocks.WATER)) {
                            world.setBlockState(pos_shift_1, Blocks.WATER.getDefaultState());
                        }
                    }
                }
            ...
            }else if (world.getBlockState(pos1).isOf(Blocks.WATER)) {
                world.playSound(player, pos1, SoundEvents.ITEM_BUCKET_EMPTY, SoundCategory.BLOCKS, 1.0F, 1.0F);
                world.setBlockState(pos1, Blocks.WATER.getDefaultState());
            }
        }
